...
public class ConfigController : ControllerBase
{
    ...
    [HttpPatch("document-types/{typeId:guid}")]
    [Authorize(Policy = "AdminOnly")]
    public async Task<IActionResult> UpdateDocumentType(Guid typeId, [FromBody] UpdateDocumentTypeCommand cmd, CancellationToken ct)
    {
        try
        {
            var dt = await _fieldMapping.UpdateDocumentTypeAsync(cmd with { DocumentTypeId = typeId }, ct);
            return Ok(dt);
        }
