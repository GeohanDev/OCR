...
using (var scope = app.Services.CreateScope())
{
    ...
    db.Database.ExecuteSqlRaw(
        "ALTER TABLE field_mapping_configs ADD COLUMN IF NOT EXISTS erp_response_field character varying(250);");
    db.Database.ExecuteSqlRaw(
        "ALTER TABLE validation_results ADD COLUMN IF NOT EXISTS erp_response_field character varying(250);");
    ...
    db.Database.ExecuteSqlRaw($@"
        ...
        VALUES ('{vsDocTypeId}', 'vendor_statement', 'Vendor Statement',
                'OcrSystem.Infrastructure.OCR.OcrPipelineService', true, NOW())
